...
**Only `montilla` (CEO / main agent) dispatches subagents** — `task(general)` (or harness equivalent) max 2 parallel in multi mode. Every other role — C-levels, reviewers, leaves — returns its deliverable to the CEO and NEVER fans out. No C-level fan-out, no sideways dispatch, no self-approval. Implementer prompts carry the no-subagents contract ("You do not dispatch subagents."); leaves never re-dispatch. Generic fallback: a harness without a subagent mechanism runs **single mode** (direct execution, no task — see Rules).
...
- "Read agent craft" → `read` on `agents/<domain>/<agent>.md` (or `skills/templates/implementers/<domain>-<level>.md` for portable implementer prompts). MANDATORY with every skill load: skill = process, template = craft. Cite path in output. Never paste full bodies of OTHER roles — only your dispatched role.
...
- "Dispatch a subagent" → `task` tool with `subagent_type="general"` (general-purpose; approved default until agents are natively registered) — **CEO-only, max 2 parallel**:
  - Prompt MUST include: (1) read stage `skills/<stage>/SKILL.md`, (2) read `agents/<domain>/<agent>.md` (or implementer prompt) for the assigned role, (3) accept `SPEC:<path>#REQ / HARD:<mode+constraints> / GATE:<verdicts> / DOMAINS:<list>` by reference only, (4) return deliverable + file list + risks + assumptions + scoped evidence.
  - Roles: only `montilla` (CEO) dispatches; C-levels return deliverables, never fan out. Canonical 8 business domains: engineering (vasquez), security (barrera), finance (dauhajre), legal (subero), marketing/brand (vera), people (santana), revenue (montero), automation/ops (espinoza + vasquez mechanics).
...
- single mode: NO task dispatch — direct execution after skill + template. multi-subagents: **CEO-only** `task(general)` max 2 parallel, each ordered to read skill + template first; C-levels return deliverables, never dispatch.
