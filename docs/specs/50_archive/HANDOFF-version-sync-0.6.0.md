# Handoff: engineering owner — Version Sync to v0.6.0

**Spec Reference:** `docs/specs/40_workspace/engineering/PROPOSED_CHANGES-version-sync-0.6.0.md` (no formal SPEC-XXX; backlog empty — ad-hoc verification + approved proposal)
...
SPEC:`docs/specs/40_workspace/engineering/PROPOSED_CHANGES-version-sync-0.6.0.md` / HARD:single (direct, no task); text-edits only; archive/history excluded / GATE:OPEN (`docs/specs/40_workspace/quality-gate/version-sync-0.6.0/GATE_REPORT.md`, 4/4 pass, `8f8281a`) / DOMAINS:[engineering]

## Deliverables

| Artifact | Location / Evidence | Status |
|----------|---------------------|--------|
| Proposal | `docs/specs/40_workspace/engineering/PROPOSED_CHANGES-version-sync-0.6.0.md` (`482dd91`) | done |
...
| Gate verdicts (4 + report) | `docs/specs/40_workspace/quality-gate/version-sync-0.6.0/` (`8f8281a`) | done |
| Evidence matrix | `qa-review.md` E-001..E-006 (Review-type, 6/6 pass) | done |
| Changelog | N/A — `CHANGELOG.md` already carries `[v0.6.0]`; this sync aligns pointers, cuts no release | N/A with justification |

## Definition of Done Checklist

- [x] Acceptance criteria satisfied — all 5 proposal rows applied; core quadruple verified untouched at 0.6.0
...
No ship-release required for this unit: v0.6.0 already shipped (`28e918d`); this sync aligns pointers to the shipped release and cuts no new version. `orchestrator` in `frame-ship:ship-release` picks this up only when the next release is cut. Rollback: `git revert 3c0d58d` (< 5 min, engineering owner).
