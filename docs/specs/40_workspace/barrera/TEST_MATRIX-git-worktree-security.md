...
| REQ-SEC-002 | T-002 | Fail-closed gate defined (`guards.md` §2) + observed state cited by reference: `git check-ignore -q .worktrees` exit=1, `.gitignore` 53 lines zero `worktrees` hits (2026-09-16) — gate trips, entry landing owned by engineering lane (C-001: cite by reference, no `.gitignore` edit in this lane) | Attestation | pass | n/a |
| REQ-SEC-003 | T-003 | Least-privilege clause (`guards.md` §3) + live state: `git worktree list` count=1 (≤2), no shared-credential copies observed; lane file create only, no cross-worktree access | Review | pass | n/a |
| REQ-SEC-004 | T-004 | Credential-shape scan over `skills/git-worktree/**` = 0 hits: tight pattern (`api_key\|passwd\|access_token\|bearer\|secret:=…\|aws_access\|ghp_\|sk-`) matched only the documented probe-pattern string in `guards.md:71` (pattern text, not a credential); PII-shape scans (SSN `\d{3}-\d{2}-\d{4}`, email) = 0 hits; broad prose matches (`secret/session/tokenize` vocabulary) are policy language, not values — C-002 met with this annotation | Review | pass | n/a |
| REQ-SEC-005 | T-005 | Submodule drill: `git submodule status` empty output exit=0 + no `.gitmodules` — none-needed statement for this repo; guard + drill steps defined in `guards.md` §5; no checkout executed — C-004 met as none-needed | Attestation | pass | n/a |
