...
| Gate evidence share (`40_workspace/<owner>/`, `quality-gate/<spec-id>/`) | Share/event: command logs, `list` snapshots, redacted excerpts, check-ignore log | internal → external — scoped-export boundary: allowlisted paths only, never full dumps/raw PII |
