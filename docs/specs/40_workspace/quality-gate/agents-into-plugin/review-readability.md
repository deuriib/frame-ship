...
**Scope:** `.opencode/plugins/frame-ship.ts` v0.6.1 (350 lines) + `.opencode/plugins/AGENTS.md` ref lines
...
| RD-003 | Low | `.opencode/plugins/frame-ship.ts:117-140` vs `163-191` | DRY / Cognitive load | Bun-first + dynamic `node:fs/promises` fallback block is duplicated in `readTextFile` and `loadBootstrapBody` (same `globalThis.Bun?.file` probe, same `@ts-ignore` dynamic import, same silent-`""` contract). Consistent with the stated "precedent" convention, so accepted for v0.6.1; a shared `readFileWithFallback(path)` helper would remove the second copy when a third reader arrives. |
...
**Fix scope verified:** commit `2966f15` touches `.opencode/plugins/AGENTS.md` (WHERE TO LOOK cells + line count) + `package.json` (0.5.0 → 0.6.1). No plugin-code change; prior CODE-lens findings stand as written.

**Cell-by-cell resolution (read back against `frame-ship.ts` @ 403 lines total):**

| Cell | Claim | Actual | Resolves? |
|------|-------|--------|-----------|
| OVERVIEW (`AGENTS.md:4`) | `v0.6.1`, 403 lines | Header `frame-ship.ts:2` = `v0.6.1`; `VERSION` L10 = `0.6.1`; `package.json:3` = `0.6.1`; file total = 403 lines | ✅ count + version quadruple (header + VERSION + MARKER L11 + manifest) aligned |
...
| Version (`:10-11`) | `VERSION` + `MARKER` bump together | L10 `VERSION = "0.6.1"` + L11 `MARKER` derived | ✅ |
