# Risk Review: version-sync-0.6.1
...
SPEC:`docs/specs/40_workspace/engineering/PROPOSED_CHANGES-version-sync-0.6.1.md` / HARD:single; text-edits only, reversible per commit; target v0.6.1 / GATE:none-yet → this verdict / DOMAINS:[engineering]
...
| R-002 | README pin forward breaks pinned installs                  | Low        | Low    | Forward-only v0.5.0 → v0.6.1; no breaking runtime change in v0.6.1 notes; revert restores pin                                       | Closed — diff shows exactly 3 pinned lines   |
