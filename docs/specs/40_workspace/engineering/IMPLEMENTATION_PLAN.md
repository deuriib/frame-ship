...
| 9 | Gate wave-2 remediation (4 conditional reviewers): RD-001 README three-plugin + guardrails truth, RD-002+QA-001 test header glob form, RD-003 proposal packet NF tail, RD-004+risk-M2 `agents.ts` comment reconciliation (logic untouched), RL-001 `withTimeout` claim scoped, QA-002+AUT-002+risk-5 revert attestation corrected, risk-M1 npmmirror provenance corrected + 5 explicit mirror URLs stripped (RL-002/AUT-003/risk-3 → cleared) | `README.md`, `tests/smoke.test.mjs`, `plugins/AGENTS.md`, `plugins/opencode/agents.ts` (comments), `package-lock.json`, `PROPOSED_CHANGES.md`, `TEST_MATRIX.md` | reviewer re-verification of own conditions; npm 10 + 11 dry-runs exit 0; JSON valid; npmmirror grep = 0 | ~30m |

Each step maps to exactly one commit (steps grouped only where same REQ): S1→`test(repo-hygiene): … (REQ-002)`, S2→`chore(repo-hygiene): … (REQ-003)`, S3→`ci(repo-hygiene): … (REQ-001)`, S4→`docs(repo-hygiene): … (REQ-004)`, S4b→`refactor(agents): … (REQ-007)` (only after the REQ-007 amendment commit), S4c→`docs(repo-hygiene): … (REQ-004 residual)` + `docs(repo-hygiene): … (REQ-005 residual)`, S5→`docs(repo-hygiene): … (REQ-005)`; S6 commits the matrix; S8→`fix(ci): … (CE-001)` (+ CE-002 unit if lockfile/matrix changes); S9→wave-2 remediation commits (docs/chore per finding class); S7 commits at ship-release. Text-only amendment commits (spec/proposal/plan/index) precede their fix commits — proposal-before-code.
