...
Each step maps to exactly one commit (steps grouped only where same REQ): S1→`test(repo-hygiene): … (REQ-002)`, S2→`chore(repo-hygiene): … (REQ-003)`, S3→`ci(repo-hygiene): … (REQ-001)`, S4→`docs(repo-hygiene): … (REQ-004)`, S4b→`refactor(agents): … (REQ-007)` (only after the REQ-007 amendment commit), S5→`docs(repo-hygiene): … (REQ-005)`; S6 commits the matrix; S7 commits at ship-release.
