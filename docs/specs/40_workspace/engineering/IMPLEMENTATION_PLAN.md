...
Each step maps to exactly one commit (steps grouped only where same REQ): S1→`test(repo-hygiene): … (REQ-002)`, S2→`chore(repo-hygiene): … (REQ-003)`, S3→`ci(repo-hygiene): … (REQ-001)`, S4→`docs(repo-hygiene): … (REQ-004)`, S5→`docs(repo-hygiene): … (REQ-005)`; S6 commits the matrix; S7 commits at ship-release.

## Order of Operations

1 (tests exist) → 2 (lockfile makes `npm ci` legal) → 3 (CI runs 1+2) → 4 (docs truthful once artifacts exist: Roadmap check-offs reference real files) → 5 (independent, any time after 4) → 6 (verify all) → 7 (only after green gate + handoff).
