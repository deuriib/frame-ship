...
## Condition Evidence — C-005, C-003, C-002 (lane parts, pathed 2026-09-16)

- C-005 TTL plus owner: every worktree declares purpose plus TTL plus deletion owner before creation, default owner automation plus security; terminal state is `git worktree remove --force` for the SPEC-ID root, then `git worktree prune`, then `git worktree list` residue verify showing the entry gone and `.worktrees/` empty or absent — pass. Statement lives in `skills/git-worktree/references/pwsh-flow.md` §5; verify steps in §§5 and 7. TTL co-sign (COND-005 automation half): repo-local `.worktrees/` is primary; fallback `Join-Path $env:TEMP "opencode"` only on repo-local refusal, with purpose plus TTL plus deletion owner automation plus security, orchestrator confirms before use — statement added in `skills/git-worktree/references/pwsh-flow.md` §3 on 2026-09-16; security co-sign of sandbox TTL still pending via S-GATE-005. T-005 pathed: contract text in `pwsh-flow.md` §5 + co-sign line in §3 (no live remove/prune run claimed — no residue created by this lane).
- C-003 live-count snapshot (T-004 pathed): command `git worktree list` on 2026-09-16 → 2 entries (`D:/GitHub/frame-ship [main]` + `D:/GitHub/frame-ship/.worktrees/DRILL-001 [drill/dry-run-001]`), count 2 at-or-below 2 — pass. Guard pinned in `skills/git-worktree/references/pwsh-flow.md` §§1, 2, 6 with the third-dispatch refusal rule. Canonical count contract by reference to ARCH INV-003.
- C-002 scan (T-006/T-007 pathed): command `rg -c '\$\(' skills/git-worktree/references/pwsh-flow.md docs/specs/40_workspace/vasquez/TEST_MATRIX-git-worktree-automation.md` on 2026-09-16 → rg-exit 1 (0 matches, 0 POSIX hits) — pass. Cross-check `Select-String -Pattern '\$\(' -Path skills/git-worktree/references/pwsh-flow.md` → Count 0. Deny-default prose mentions of secret and credential words are policy statements, not embedded values. Full trust-boundary map and remaining C-002, C-001, C-004, C-006 parts by reference to security lane and orchestrator.
- T-002 serialized-setup pathed: installer-mutex kill bound added in `skills/git-worktree/references/pwsh-flow.md` §3 on 2026-09-16 (max 10-min bounded wait, then STOP with orchestrator notify, never kills another lane's installer); toolchain reuse check `git diff -- mise.toml` on 2026-09-16 → 0 lines, exit 0 (`mise.toml` `[tools] node = "22"` intact, no live `mise install` overlap created by this lane) — conditional-pass, live timestamped setup logs still pending orchestrator-aggregated dry run.
- T-003 baseline-gate pathed: ignore-gate pre-check `git check-ignore -q .worktrees/` on 2026-09-16 → exit 0 (proceed) — pass for gate entry; `mise run typecheck` green-in-worktree drill still pending (no live red/green drill claimed here) — conditional-pass.
