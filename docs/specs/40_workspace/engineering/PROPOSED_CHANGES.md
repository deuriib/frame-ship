...
**Spec Reference:** SPEC-repo-hygiene (`docs/specs/20_backlog/SPEC-repo-hygiene.md#REQ-001..006+REQ-NF-001..003`)
...
| `package.json` | file-modify | Add `"test": "node --test tests/"`. `dependencies` untouched (`@opencode/plugin@2.0.9` only) (REQ-003). |
...
| `plugins/AGENTS.md` | file-modify | Version-lockstep example `const VERSION = "0.8.0"` → version-neutral wording pointing at `shared.ts` (no stale hardcoded version to rot again) (REQ-004). |
