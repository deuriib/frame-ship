...
- REQ-003 (test entry point): `package.json` gains `"test": "node --test tests/"`; dependency set unchanged (`@opencode/plugin@2.0.9` only) + `package-lock.json` committed so `npm ci` pins it.
...
- [ ] AC-004: `grep -rn "hooks/context-inject" README.md` → 0 hits; raw tsc command matches `mise.toml` file list; roadmap claims match repo state; evidence grep output in `TEST_MATRIX.md`.
