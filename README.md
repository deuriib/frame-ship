...
Then quit + restart opencode (config is not hot-reloaded). Verify: the system prompt contains `[frame-ship v0.5.0]` and the native `skill` tool discovers `using-frame-ship` through `ship-release`.
...
├── rules/
│   └── frame-ship.md             # persistent cards, verbatim, version-locked v0.5.0
